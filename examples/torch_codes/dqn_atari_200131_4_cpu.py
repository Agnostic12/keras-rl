...
1. Test all the process only in cpu. To compare speed

Conclusion: only CPU is slower.
"""
import os
...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.from_numpy(x).permute(0, 3, 2, 1).to(torch.float32)
        x = self.conv1(x)
        x = F.relu(x)
        x = self.conv2(x)
        x = F.relu(x)
...
class DQNAgent(object):
    ...
    def update(self, sampled_memory, counter=None, writer=None):
        ...
        td_error_tensor = torch.from_numpy(td_target) - \
                          torch.gather(pred_state_vals, 1, torch.from_numpy(actions))
