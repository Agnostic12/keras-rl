...
1. convert data into tensors before we append them to memory.
2. add save_model module

Conclusion: It doesn't work. If you put too many things into GPU, it will OOM.
"""
import os
...
class DQNAgent(object):
    ...
    def update(self, sampled_memory, counter=None, writer=None):
        ...
        pred_next_state_vals = self.net.forward(next_states)
        pred_state_vals = self.net.forward(states)
        td_target = rewards + \
                    self.gamma * torch.max(pred_next_state_vals, dim=1, keepdims=True).values * (~dones).to(torch.float32)
