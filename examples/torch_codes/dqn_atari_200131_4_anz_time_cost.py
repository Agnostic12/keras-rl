...
class Net(nn.Module):
    def __init__(self, env, lp):
        ...
        self.lp.add_function(self.forward)
...
class DQNAgent(object):
    def __init__(self, net, memory, gamma, lp):
        self.net = net
        self.memory = memory
        self.gamma = gamma
        self.optimizer = optim.Adam(self.net.parameters())
        self.lp = lp
        self.lp.add_function(self.eps_greedy_action)
        self.lp.add_function(self.update)
...
def train():
    ...
    net = Net(env, lp).cuda()
    dqn = DQNAgent(net, memory, 0.9, lp)
    lp_wrapper = lp(dqn.eps_greedy_action)
    lp_wrapper2 = lp(dqn.update)
    # to clear logs
    ...
    for i_episode in range(n_episodes):
        ...
        for t in count():
            ...
            raw_action = lp_wrapper(state, eps=curr_eps)
            ...
            if memory.size >= batch_size and global_counter % train_every_n == 0 :
                ...
                lp_wrapper2(sampled_memory, counter=global_counter, writer=writer)
